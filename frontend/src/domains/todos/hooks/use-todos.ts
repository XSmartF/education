...
import { todosQueryKeys } from '../model/query-keys';
...
export function useTodos(): UseTodosResult {
  ...
  const addMutation = useMutation({
    ...
    onSuccess: (created) => {
      queryClient.setQueryData<TodoItem[]>(todosQueryKeys.all, (prev = []) => [created, ...prev]);
    },
  });
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: (_, item) => {
      queryClient.setQueryData<TodoItem[]>(todosQueryKeys.all, (prev = []) =>
  ...
  return {
    ...
    error: errorMessage,
    refresh: () => queryClient.invalidateQueries({ queryKey: todosQueryKeys.all }).then(() => undefined),
